...
C LBFGS
         ...
         if(.not.exist)then
          ...
          do if=1,msite(1)+msite(2)
           ...
           read(3000,*) frozen(if)
           f(1,if)=f(1,if)*dble(frozen(if))
           f(2,if)=f(2,if)*dble(frozen(if))
           f(3,if)=f(3,if)*dble(frozen(if))
          end do
          close(90)
          close(3000,status='delete')
         end if
!Append force to records file
...
c        print*,'istat:',istat
         ...
         do if=1,msite(1)+msite(2)
           do j=1,3
             nopt=nopt+1
             !YY conditional added to prevent updating frozen FODs.
             if(frozen(if) .ne. 0) r(j,if)=xvec(nopt)
             !r(j,if)=xvec(nopt)
           end do
           if(exist) then
            write(90,99)(r(j,if),j=1,3),frozen(if),
     &                   d2inv(if),suggest(if)
           else
...
c        call system('echo "0  " >> RUNS')
 ...
 99      format(3d20.12,I3,'  D2INV=',F12.4,' SUGGEST= ',F12.4)
         end
