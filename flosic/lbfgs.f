...
C     ****************
C
      SUBROUTINE LBFGS(N,M,X,F,G,DIAGCO,DIAG,IPRINT,EPSL,XTOL,W,
     & IFLAG,DGUESS)
C
...
C IF GRADIENTS HAVE CONVERGED THEN EXIT
        ...
        INQUIRE(FILE='SEARCH.LBF',EXIST=EXIST)
        IF (EXIST) THEN
         OPEN(UNIT=7,FILE='SEARCH.LBF',STATUS='OLD')
         READ(7,*)ITER
         READ(7,*)INFO
         READ(7,*)NFEV
         READ(7,*)ISPT
...
C     ----------------------------------------------------
 ...
 170  W(I)=G(I)
 172  CONTINUE
      CALL MCSRCH(N,X,F,G,W(ISPT+POINT*N+1),STP,FLTOL,
     &            XTOL,MAXFEV,INFO,NFEV,DIAG)
*****************CHANGES MADE HERE*****************
        OPEN(UNIT=7,FILE='SEARCH.LBF',STATUS='UNKNOWN')
        WRITE(7,*)ITER, " ITER"
        WRITE(7,*)INFO, " INFO"
        WRITE(7,*)NFEV, " NFEV"  
        WRITE(7,*)ISPT, " ISPT"
...
C
 ...
 300   IF ((IFLAG .EQ. 0) .OR. (N .EQ. 0)) THEN
        OPEN(UNIT=4,FILE='DIAG.LBF',FORM='formatted',STATUS='unknown')
        CLOSE(4,STATUS='delete')
        OPEN(UNIT=7,FILE='SEARCH.LBF',FORM='formatted',STATUS='unknown')
        CLOSE(7,STATUS='DELETE')
        OPEN(UNIT=8,FILE='STEP.LBF', FORM='formatted',STATUS='unknown')
