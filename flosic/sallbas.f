...
C ******************************************************************
C
       SUBROUTINE SALLBAS(IZNUC,ALP,CON,NALP,NBASF)
C
C DIRK POREZAG, AUGUST 1997
C BASIS SET GENERATOR FOR ALL-ELECTRON CALCULATIONS
C DATA STORED IN COMMON BLOCK BDALLBAS
C
C ATTENTION: PARAMETERS MXCHG,MXBAR,MXCON MUST BE IDENTICAL IN
C BDALLBAS AND SALLBAS
C
...
!      INCLUDE  'PARAMAS'  
     ...
     & IALP, IBOFS, IOFS, L, NADD, NFULL, NG, NMAX, NSPD
       ...
       PARAMETER (MXBAR=25)
       PARAMETER (MXCON=6)
       COMMON/ALLBAS/ALPH(MXBAR,MXCHG),COEF(MXBAR,MXCON,3,MXCHG)
...
C
       IF (IZNUC.GT.MXCHG) THEN
        PRINT *,'SALLBAS: AUTOMATED BASIS SETS NOT AVAILABLE '
     &         ,'FOR Z > ',MXCHG
        CALL STOPIT
       END IF
       IF (IZNUC.LT.0) THEN 
...
C
       IF (IZNUC.EQ.0) THEN
        ...
        IF (NALP.GT.MAX_BARE) THEN
         PRINT *,'SALLBAS: MAX_BARE MUST BE AT LEAST: ',NALP
         CALL STOPIT
        END IF
        ALP(1)=1.50D0
        ALP(2)=0.60D0
        ...
        IF (NMAX.GT.MAX_CON) THEN
         PRINT *,'SALLBAS: MAX_CON MUST BE AT LEAST: ',NMAX
         CALL STOPIT
        END IF
        GOTO 100
       END IF
...
C
       ...
       IF (NMAX.GT.NALP) THEN
        PRINT *,'SALLBAS: INTERNAL ERROR: LINEAR DEPENDECY'
        CALL STOPIT
       END IF
       DO IALP=1,NALP
  ...
  100  DO L=1,3
        ...
        DO IADD=1,2
         IF ((NADD(IADD,L)+IBOFS(IADD,L)).GT.NALP) THEN
          PRINT *,'SALLBAS: INTERNAL ERROR: NADD+IBOFS TOO LARGE'
          CALL STOPIT
         END IF
         DO I=1,NADD(IADD,L)
          DO IALP=1,NALP
